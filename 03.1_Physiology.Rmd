...
#### PERMANOVA
Checking for an overall effect of Origin or Site, along with interactions between Origin and Site and Origin with Time. Time is not included as a main individual effect as seasonal changes in physiology are not the focus of the analysis. Rather, the effect of time since transplant on the effect of origin is of interest.
```{r}
##PERMANOVA
Coral.perm<-adonis2(vegdist(Coral_St[,c(11:13)], "euclidean")~ Coral_St$Origin + Coral_St$Site + Coral_St$Origin:Coral_St$Site + Coral_St$Origin:Coral_St$TimeP, data=Coral_St, strata=Coral_St$Genotype, method="euclidean")
...
##Save results
PERM.res<-data.frame(adonis_OmegaSq(Coral.perm)[1:4,])
PERM.res$Predictor<-c("Origin", "Site", "Origin x Site", "Origin x Time")
PERM.res$p_DISP<-c(Orig_disp$`Pr(>F)`[1], Site_disp$`Pr(>F)`[1], Orig.Site_disp$`Pr(>F)`[1], Orig.Time_disp$`Pr(>F)`[1])
...
TP1_Prot.plot<-ggplot(TP1_Prot.sum, aes(x=Site, y=TP_ug.cm2, colour=Site.Orig)) + 
  ...
  ylim(550, 950)+
  annotate("text", x=2, y=950, label="**", size=sig.sz, fontface="bold"); TP1_Prot.plot
```
...
Bio.ES.plot<-ggplot(Phys.ES[which(Phys.ES$Metric=="AFDW_mg.cm2"),], aes(x=TimeP, y=EtaSq, fill=Site))+
...
Prot.ES.plot<-ggplot(Phys.ES[which(Phys.ES$Metric=="TP_ug.cm2"),], aes(x=TimeP, y=EtaSq, fill=Site))+
  geom_bar(stat="identity", position=position_dodge())+
    scale_fill_manual(values=Site.colors.o)+
theme_classic()+
  ggtitle("Protein")+
