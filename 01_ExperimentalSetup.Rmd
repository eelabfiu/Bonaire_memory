...
##Rename Variables
EnvData.clean<-EnvData.cut %>% dplyr::rename("Avg Temp" = Temp.a, "Avg Cond" = Cond.a, "Temp SD" = Temp.sd, "Cond SD" = Cond.sd, "Sat PAR" = Sat.hr, "N:P" = N.P, "DIN" = DIN_uM)

##PCA
Env.pca<-prcomp(EnvData.clean[,-c(1:2)], scale=TRUE)

summary(Env.pca)

##Plot PCA
Env.pca.plot<-autoplot(Env.pca, data=EnvData.cut, colour="Site", loadings=TRUE,
         loadings.label=TRUE, loadings.colour="black", loadings.label.colour="black", loadings.label.size=sig.sz, size=point.sz)+
  ...
  theme_classic()+
   theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        legend.title=element_text(size=leg.title.sz), 
        legend.box.background = element_rect(color = "black"))+
  annotate("text", x=0.01, y=-0.32,label = "Site PERMANOVA p=0.018", size=sig.sz-1, fontface="bold.italic", hjust=0)+
  annotate("text", x=0.01, y=-0.37,label = "Site PERMDISP p=0.507", size=sig.sz-1, fontface="italic", hjust=0); Env.pca.plot

```


```{r}
fviz_pca_biplot(Env.pca, repel=TRUE, geom = "point", col.var="black", labelsize=sig.sz)+
  geom_point(aes(colour=EnvData.clean$Site),size=point.sz)+
  scale_colour_manual(values =Site.colors.o)+
  theme_classic()+
   theme(axis.title.x = element_text(size = axis.title.sz), 
         axis.title.y = element_text(size = axis.title.sz), 
...
### Figure S1 Environmental Conditions
...
##Save Figure
ggsave(filename="Figures/01_Design/FigS1_Environmental_Conditions.png", plot=Env.Param_fig, dpi=300, width=10, height=16, units="in")

```


### Figure S2B Thermal Tolerance Temperatures
