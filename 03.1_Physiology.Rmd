...
#### Adjust for Panel
```{r}
Phys.Site.Orig.ES.plot<-Phys.Site.Orig.ES.plot+ labs(x="")
```
...
```{r}
##Combine Results of TP Models
Phys.pair.res_TP<-rbind(Phys.pair_TP1.res, Phys.pair_TP2.res, Phys.pair_TP3.res, Phys.pair_TP4.res)

##Rename p.value for merging
Phys.pair.res_TP<-Phys.pair.res_TP %>% dplyr::rename(p_Timepoint = p.value)

##Combine Results Tables
Phys.pair.res.all<-merge(Phys.pair.res, Phys.pair.res_TP[,c("Site", "TimeP", "Response", "p_Timepoint")])
TableS5_Phys.Pair<-Phys.pair.res.all

##Organize
names(TableS5_Phys.Pair)
TableS5_Phys.Pair<-TableS5_Phys.Pair %>% dplyr::rename(Timepoint = TimeP, Contrast =contrast, Estimate = estimate, DF = df, t = t.ratio, p = p.value, EffectSize = effect.size, EffectSize.SE = SE.es)
TableS5_Phys.Pair<-TableS5_Phys.Pair[,c("Timepoint", "Response", "Site", "Contrast", "Estimate", "SE", "DF", "t", "p", "p_Timepoint", "EffectSize", "EffectSize.SE")]

#Round to 3 digits
TableS5_Phys.Pair$Estimate<-round(TableS5_Phys.Pair$Estimate, 3)
TableS5_Phys.Pair$SE<-round(TableS5_Phys.Pair$SE, 3)
TableS5_Phys.Pair$t<-round(TableS5_Phys.Pair$t, 3)
TableS5_Phys.Pair$p<-round(TableS5_Phys.Pair$p, 3)
TableS5_Phys.Pair$p_Timepoint<-round(TableS5_Phys.Pair$p_Timepoint, 3)
TableS5_Phys.Pair$EffectSize<-round(TableS5_Phys.Pair$EffectSize, 3)
TableS5_Phys.Pair$EffectSize.SE<-round(TableS5_Phys.Pair$EffectSize.SE, 3)
