...
T1.2_TLE_Sym.plot<-ggplot(T1.2_Perf_Set, aes(x=TLE_cm.day, y=Sym.prop)) + 
  ...
  labs(x=expression(paste('Growth Rate (cm day'^-1*")")), 
       ...
       colour="Site Origin")+
   annotate("text", x = 0.07, y = 0, label=expression(bolditalic(paste(r[S], " = -0.615, p = 0.037"))), size=sig.sz, hjust = 0); T1.2_TLE_Sym.plot
```
...
T1.2_TLE_Chl.plot<-ggplot(T1.2_Perf_Set, aes(x=TLE_cm.day, y=Chl.prop)) + 
  ...
  labs(x=expression(paste('Growth Rate (cm day'^-1*")")), 
       ...
       colour="Site Origin")+
   annotate("text", x = 0.07, y = 0, label=expression(bolditalic(paste(r[S], " = -0.587, p = 0.049"))), size=sig.sz, hjust = 0); T1.2_TLE_Chl.plot
```
...
T1.2_TLE_PAM.plot<-ggplot(T1.2_Perf_Set, aes(x=TLE_cm.day, y=PAM.prop)) + 
  ...
  labs(x=expression(paste('Growth Rate (cm day'^-1*")")), 
       ...
       colour="Site Origin")+
   annotate("text", x = 0.07, y = 0.74, label=expression(bolditalic(paste(r[S], " = -0.678, p = 0.019"))), size=sig.sz, hjust = 0); T1.2_TLE_PAM.plot
```
...
T3.4_TLE_Sym.plot<-ggplot(T3.4_Perf_Set, aes(x=TLE_cm.day, y=Sym.prop)) + 
  ...
  labs(x=expression(paste('Growth Rate (cm day'^-1*")")), 
       y="Symbiont Retention", 
       colour="Site Origin")+
   annotate("text", x = 0.07, y = 0, label=expression(italic(paste(r[S], " = -0.552, p = 0.067"))), size=sig.sz, hjust = 0); T3.4_TLE_Sym.plot
```
...
T3.4_TLE_Chl.plot<-ggplot(T3.4_Perf_Set, aes(x=TLE_cm.day, y=Chl.prop)) + 
  ...
  labs(x=expression(paste('Growth Rate (cm day'^-1*")")), 
       y="Chlorophyll Retention", 
       colour="Site Origin")+
   annotate("text", x = 0.07, y = 0, label=expression(italic(paste(r[S], " = 0.327, p = 0.327"))), size=sig.sz, hjust = 0); T3.4_TLE_Chl.plot
```
...
TP3.4_Growth.plot.cut<-ggplot(TP3.4_Growth.sum, aes(x=Site, y=TLE_cm.day, colour=Site.Orig)) + 
  scale_colour_manual(values=Orig.colors.o)+
  geom_errorbar(aes(ymin=TLE_cm.day-se, ymax=TLE_cm.day+se), width=cap.sz, linewidth=bar.sz, position=position_dodge(width=0.5)) +
  geom_point(size=point.sz, position=position_dodge(width=0.5))+
  theme_classic()+
...
```{r}
##Create Panel
Growth_fig<-plot_grid(TP1.2_Growth.plot.cut, TP3.4_Growth.plot.cut, 
                    rel_widths=1, rel_heights = 1,
                    nrow=1, ncol=2, byrow=T, labels = c("A", "B"))

##Save Figure
ggsave(filename="Figures/03_Performance/Fig3_Growth.png", plot=Growth_fig, dpi=300, width=10, height=4, units="in")

```



### Figure 4 Tolerance ES
...
```{r}
##Create Panel
Tolerance_fig<-plot_grid(Tol_PAM.ES.plot.cut, Tol_Sym.ES.plot.cut, 
                         Tol_Chl.ES.plot.cut,
                    rel_widths=1, rel_heights = 1,
                    nrow=1, ncol=3, byrow=T, labels = c("A", "B", "C"))

##Save Figure
ggsave(filename="Figures/03_Performance/Fig4_Tolerance.png", plot=Tolerance_fig, dpi=300, width=14, height=4, units="in")
...
#### Adjust Figures for Panel
```{r}
##Growth vs FvFm Retention T1.2
T1.2_TLE_PAM.plot.cut<-ggplot(T1.2_Perf_Set, aes(x=TLE_cm.day, y=PAM.prop)) + 
   geom_smooth(method="lm", color="#AA185AFF", fill="#F8D7BFFF", se=TRUE)+
   geom_point(aes(colour=Site.Orig),size=point.sz)+
    scale_colour_manual(values=Orig.colors.o)+
   theme_classic()+
...
##Growth vs Symbiont Retention T1.2
T1.2_TLE_Sym.plot.cut<-ggplot(T1.2_Perf_Set, aes(x=TLE_cm.day, y=Sym.prop)) + 
   geom_smooth(method="lm", color="#AA185AFF", fill="#F8D7BFFF", se=TRUE)+
   geom_point(aes(colour=Site.Orig),size=point.sz)+
    scale_colour_manual(values=Orig.colors.o)+
   theme_classic()+
...
##Growth vs FvFm Retention T3.4
T3.4_TLE_PAM.plot.cut<-ggplot(T3.4_Perf_Set, aes(x=TLE_cm.day, y=PAM.prop)) + 
   geom_smooth(method="lm", color="#AA185AFF", fill="#F8D7BFFF", se=TRUE)+
   geom_point(aes(colour=Site.Orig),size=point.sz)+
    scale_colour_manual(values=Orig.colors.o)+
   theme_classic()+
  ...
  labs(x=expression(paste('Growth Rate (cm day'^-1*")")), 
       y="Photo. Effic. Retention", 
       colour="Site Origin")+
   annotate("text", x = 0.07, y = 0.5, label=expression(bolditalic(paste(r[S], " = -0.748, p = 0.007"))), size=sig.sz, hjust = 0)

##Growth vs Symbiont Retention T3.4
T3.4_TLE_Sym.plot.cut<-ggplot(T3.4_Perf_Set, aes(x=TLE_cm.day, y=Sym.prop)) + 
   geom_smooth(method="lm", color="#AA185AFF", fill="#F8D7BFFF", se=TRUE)+
   geom_point(aes(colour=Site.Orig),size=point.sz)+
    scale_colour_manual(values=Orig.colors.o)+
   theme_classic()+
  ...
  labs(x=expression(paste('Growth Rate (cm day'^-1*")")), 
       y="Symbiont Retention", 
       colour="Site Origin")+
   annotate("text", x = 0.07, y = 0, label=expression(italic(paste(r[S], " = -0.552, p = 0.067"))), size=sig.sz, hjust = 0)
...
```{r}
##Create Panel
TradeOff_fig<-plot_grid(T1.2_TLE_PAM.plot.cut, T1.2_TLE_Sym.plot.cut, T1.2_TLE_Chl.plot,
                        T3.4_TLE_PAM.plot.cut, T3.4_TLE_Sym.plot.cut, T3.4_TLE_Chl.plot,
                    rel_widths=c(.75, .75, 1,.75, .75, 1), 
                    rel_heights = 1,
                    nrow=2, ncol=3, byrow=T, labels = c("A", "B", "C", "D", "E", "F"))

##Save Figure
ggsave(filename="Figures/03_Performance/Fig4_TradeOff.png", plot=TradeOff_fig, dpi=300, width=12, height=8, units="in")
...
TableS5<-data.frame(rbind(
   summary(TLE_T1.2.lme)$coefficients[-1,],summary(TLE_T1.2.lme_KL)$coefficients[-1,],
   summary(TLE_T1.2.lme_SS)$coefficients[-1,],
   summary(TLE_T3.4.lme)$coefficients[-1,],summary(TLE_T3.4.lme_KL)$coefficients[-1,],
...
##Metadata 
TableS5$Type<-c(rep("Growth", 10), rep("Tolerance", 60))
TableS5$Metric<-c(rep("Growth", 10), rep(c(rep("FvFm", 5), rep("Symbionts", 5), rep("Chlorophyll", 5)), 4))
TableS5$TimeP<-c(rep("TP1.2", 5),rep("TP3.4", 5), rep("TP1",15), rep("TP2",15), rep("TP3",15), rep("TP4",15))
TableS5$Predictor<-c(rep(c("Origin", "Site", "Origin x Site", "KL Origin", "SS Origin"), 14))

##Effect Size  
TableS5$EtaSq<-c(
  eta_squared(TLE_T1.2.lme)$Eta2_partial,eta_squared(TLE_T1.2.lme_KL)$Eta2_partial,
   eta_squared(TLE_T1.2.lme_SS)$Eta2_partial,
  eta_squared(TLE_T3.4.lme)$Eta2_partial,eta_squared(TLE_T3.4.lme_KL)$Eta2_partial,
   eta_squared(TLE_T3.4.lme_SS)$Eta2_partial,
